...
意図的に公開していない内部ドキュメントが 1 つあります: エンジニアリング・ナレッジベース ─ このコードベースと外部サービスについての「検証済みの学び」と「未検証の仮説」を、検証ステータスで厳密に分離して蓄積する記録です。インシデントの詳細や修正前のセキュリティメモを含み得るため非公開としています。安定した規約に固まったルールは [AGENTS.md](./AGENTS.md) に反映されます。
