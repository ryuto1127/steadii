...
The 令和トラベル interview email is the canonical test case:
...
Test case: 令和トラベル — JST sender, PDT student, 30-minute interview slot. Draft body must show both TZs side-by-side for any candidate slot it proposes.
...
3. **Live dogfood via preview**:
   - Log in as Ryuto's account, find the 令和トラベル row
