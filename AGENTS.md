...
- `notion_connections` (Notion workspace + encrypted access token + the four Steadii DB IDs — `classes_db_id`, `mistakes_db_id`, `assignments_db_id`, `syllabi_db_id` — plus `parent_page_id` and `setup_completed_at`)
- `registered_resources` (Notion pages/DBs the agent can access; `archived_at` is used by auto-discovery for soft-removing resources that disappear from the Steadii parent)
...
- `audit_log` (agent actions on external resources — Notion writes, calendar edits, auto-discovery diffs)

### 4.1 Notion data model (4 databases)

Under the Steadii parent page, four Notion databases are created in this order — **Classes first** because the other three reference it:

1. **Classes** — the class-centric hub. Properties: `Name` (title), `Code` (rich_text), `Term` (select), `Professor` (rich_text), `Color` (select), `Status` (select: active / archived, default active). See PRD §3.5.
2. **Mistake Notes** — per PRD §3.6.3.
3. **Assignments** — per PRD §3.7.1.
4. **Syllabi** — per PRD §3.8.3.

Mistake Notes, Assignments, and Syllabi each have a `Class` property that is a **relation to Classes with `dual_property` two-way sync**. Every agent tool that filters, groups, or reports on these three tables **must join through the `Class` relation** rather than keying on an ad-hoc string — the canonical identity of a class is its Classes-DB page ID, not its name or code.

Auto-discovery (PRD §3.9) keeps `registered_resources` in lockstep with the direct children of the Steadii parent page: new children are inserted with `autoRegistered=true`, resources that vanish from Notion get `archived_at` set (no hard delete), and the discovery pass is cached per-user for 60 seconds.
